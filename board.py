    ...
    def direccionv2(self):
        ...
        while not exitGame:
            while True:
                ...
                if not isinstance(self.enter_index,int) or self.enter_index > len(self.turn[self.turnCount]) or str(self.enter_index) == '':
                    system('cls')
                    print('enter a number that is in the range of your card or entered a letter')
                    print('reenter the index')  
            
                elif self.turnCount < len(self.turn):
                    # print(self.turn[self.turnCount][self.enter_index])
                    self.index_v.append(self.turn[self.turnCount][self.enter_index])
                    if self.status == False:
                        self.insert_token6_in_table(self.table)
                        self.turnCount += 1
                    elif self.cheeck_table_in_turn(self.table,self.turn[self.turnCount]):
                        if self.status == True:      
                            self.insert_tokens_in_table(self.table) 
                    elif not self.cheeck_table_in_turn(self.table,self.turn[self.turnCount]):
                        print('the Player{}'.format(self.turnCount+1), 'is no tiene token, re-enter index')
                        self.index_v.pop()
                        self.turnCount     
